...
/**
 * Tags to use the standard output stream (stderr) or error output stream (stderr).
 */
inline constexpr struct output_stream { bool err; } stdout{false}, stderr{true};
...
/**
 * Prints all arguments in a simple, standardized format to a specific output stream. Each argument is separated by one
 * space ending with a line break.
 * @param stream - the output stream (stdout/stderr)
 * @param args - the arguments to print
 */
template<typename OutputStream,
         typename... Args,
         std::enable_if_t<std::is_same_v<std::decay_t<OutputStream>, output_stream>>* = nullptr>
constexpr auto print(OutputStream&& stream, Args&&... args);

/**
 * Formats and prints all arguments in the desire format.
...
/**
 * Formats and prints all arguments in the desire format to a specific output stream.
 * @param stream - the output stream (stdout/stderr)
 * @param args - the arguments to format and print
 */
template<typename OutputStream,
         typename... Args,
         std::enable_if_t<std::is_same_v<std::decay_t<OutputStream>, output_stream>>* = nullptr>
constexpr auto printf(OutputStream&& stream, std::string_view format = "", Args&&... args);

/**
 * For user-defined types, the format method of the specialized formatter<T> struct template is used.
 * Provide method `constexpr auto format(T);` returning a tuple like object. The first element must be a format
 * string followed by the arguments to format.
...
/**
 * Takes any contiguous container or range.
...
/**
 * Wrapper to print the types of arguments rather than the values.
...
/**
 * Helper to generate extra noise to force compiler to print.
 */
struct noise {};
...
constexpr void print_start_indicator(int& one, Arg&& arg, Args&&...) {
	if constexpr (std::is_same_v<std::decay_t<Arg>, output_stream>) {
		if (arg.err) {
			if (Format) {
...
constexpr void print_helper(int& one, std::index_sequence<Is...> /*unused*/, Args&&... args) {
	...
	// The separator is used to better parse the type of the first parameter from the variadic template list.
	// All other parameters and the index sequence are unused but otherwise same arguments are mentioned only once.
...
template<bool Format,
         ...
         typename... Args,
         std::enable_if_t<!std::is_same_v<std::decay_t<Arg>, output_stream> ||
                          (std::is_same_v<std::decay_t<Arg>, output_stream> && sizeof...(Args) > 0)>* = nullptr>
constexpr auto print(Arg&& arg = {}, Args&&... args) {
	int one = 1;
	print_start_indicator<Format>(one, std::forward<Arg>(arg), std::forward<Args>(args)...);

	/// Skip first argument if output stream.
	if constexpr (std::is_same_v<std::decay_t<Arg>, output_stream>) {
...
template<typename OutputStream,
         typename... Args,
         std::enable_if_t<std::is_same_v<std::decay_t<OutputStream>, output_stream>>*>
constexpr auto print(OutputStream&& stream, Args&&... args) {
	return detail::print<false>(std::forward<OutputStream>(stream), std::forward<Args>(args)...);
}
...
template<typename OutputStream,
         ...
         std::enable_if_t<std::is_same_v<std::decay_t<OutputStream>, output_stream>>*>
constexpr auto printf(OutputStream&& stream, std::string_view format, Args&&... args) {
	return detail::print<true>(std::forward<OutputStream>(stream), format, std::forward<Args>(args)...);
}
...
public:
	...
	constexpr view(const T* first, const T* last) : data_{first}, size_{last - first} {}
