...
def compile_print_call(args, format=False, funct_scope='', global_scope='', pre_include=''):
    if format:
        call = 'printf'
    else:
        call = 'print'
    args = [str(x) for x in args]
    return compile_file(cpp_file.format(pre_include, global_scope, funct_scope, call, ', '.join(args)))
...
def test_char():
    ...
    log = compile_print_call(['c'], funct_scope='const char& c = 32;')
    assert_printers(log, [(False, sys.stdout, [' '])])
...
def test_array():
    log = compile_print_call(['x'], funct_scope='int x[] = {1, 2, 3};')
    assert_printers(log, [(False, sys.stdout, [[1, 2, 3]])])

    log = compile_print_call(['std::array<int, 3>{{1, 2, 3}}'])
    assert_printers(log, [(False, sys.stdout, [[1, 2, 3]])])

    log = compile_print_call(['x'], funct_scope='std::array<int, 3> x{{1, 2, 3}};')
    assert_printers(log, [(False, sys.stdout, [[1, 2, 3]])])

    log = compile_print_call(['x'], funct_scope='const std::array<int, 3> x{{1, 2, 3}};')
    assert_printers(log, [(False, sys.stdout, [[1, 2, 3]])])

    log = compile_print_call(['x'], funct_scope='std::array<std::array<int, 1>, 3> x{{1, 2, 3}};')
    assert_printers(log, [(False, sys.stdout, [[[1], [2], [3]]])])

    log = compile_print_call(['x'], funct_scope='std::array<std::array<std::array<int, 1>, 1>, 3> x{{1, 2, 3}};')
    assert_printers(log, [(False, sys.stdout, [[[[1]], [[2]], [[3]]]])])

    log = compile_print_call(['x'], funct_scope='std::array<std::string_view, 3> x{{{"ccdd"}, {"cd"}, {"cd"}}};')
...
def test_view():
    ...
    log = compile_print_call(['ctp::view(x)'], funct_scope='int x[] = {1, 2, 3};')
    assert_printers(log, [(False, sys.stdout, [[1, 2, 3]])])

    log = compile_print_call(['ctp::view(x, 2)'], funct_scope='int x[] = {1, 2, 3};')
    assert_printers(log, [(False, sys.stdout, [[1, 2]])])

    log = compile_print_call(['ctp::view(x, 0)'], funct_scope='int x[] = {1, 2, 3};')
    assert_printers(log, [(False, sys.stdout, [[]])])

    log = compile_print_call(['ctp::view(x + 1, x + 2)'], funct_scope='int x[] = {1, 2, 3};')
    assert_printers(log, [(False, sys.stdout, [[2]])])

    log = compile_print_call(['ctp::view(x, x)'], funct_scope='int x[] = {1, 2, 3};')
    assert_printers(log, [(False, sys.stdout, [[]])])
    ...
    """
    log = compile_print_call(['a'], funct_scope='A a{};', global_scope=user_defined_container)
...
def test_tuple_like():
    ...
    log = compile_print_call(['x'], funct_scope='std::tuple x{1, 2.0, "hello"};')
    assert_printers(log, [(False, sys.stdout, [(1, 2.0, 'hello')])])

    log = compile_print_call(['x'], funct_scope='std::tuple x{"abc", std::tuple{2.0}, std::tuple{-5.123}};')
